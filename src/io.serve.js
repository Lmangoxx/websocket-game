    ...
    init () {
        io.on('connection', socket => {
            ...
            if (socket) {
                socket.on('join', (user) => {
                    // 用户加入时
                    this.GROUP[socket.id] = {
                        id: socket.id,
                        sex: 0
                    }
                    Object.assign(this.GROUP[socket.id], user || {}, {
                        type: 'join',
                        time: new Date()
                    });
                    socket.emit('getUserInfo', this.GROUP[socket.id]);
                    io.emit('changeUsers', this.GROUP);
                    socket.broadcast.emit('message', this.GROUP[socket.id]);
                    
                    socket.on('message', (data) => {
                        io.emit('message', Object.assign(this.GROUP[data.id], {
                            type: 'message',
                    ...
                    socket.on('disconnect', () => {
                        socket.broadcast.emit('message', Object.assign(this.GROUP[socket.id], {
                            type: 'leave',
                            time: new Date()
                        }));
                        delete this.GROUP[socket.id];
                        socket.broadcast.emit('changeUsers', this.GROUP);
                    });
                })
            }
        })
